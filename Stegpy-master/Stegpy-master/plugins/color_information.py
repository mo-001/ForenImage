    ...
    def process(self, image):
        ...
        if image.mode == 'RGBA':
            print('  Red distribution :     %s' % \
                    str(list(set([r for r,g,b,a in list(colors.values())]))))
            print('  Green distribution :   %s' % \
                    str(list(set([g for r,g,b,a in list(colors.values())]))))
            print('  Blue distribution :    %s' % \
                    str(list(set([b for r,g,b,a in list(colors.values())]))))
            print('  Alpha distribution :   %s' % \
                    str(list(set([a for r,g,b,a in list(colors.values())]))))
        else:
            stat = image.convert('RGB')
            print('  Red distribution :     %s' % \
                    str(list(set([r for r,g,b in list(colors.values())]))))
            print('  Green distribution :   %s' % \
                    str(list(set([g for r,g,b in list(colors.values())]))))
            print('  Blue distribution :    %s' % \
                    str(list(set([b for r,g,b in list(colors.values())]))))
